...
// Contract designed to hold and stake Generic ERC20 Token
contract GenericERC20Stake is ReentrancyGuard {
    ...
    constructor(address ge20Address) {
        GE20 = ge20Address;
    }
    ...
    function deposit(uint256 amount, string calldata waxAddress) external nonReentrant returns(uint256, uint256){
        require(IGenericERC20(GE20).balanceOf(msg.sender) > 0, 'cannot stake without coin');
        require(IGenericERC20(GE20).balanceOf(msg.sender) >= amount, 'cannot stake more than you have');
        require(bytes(waxAddress).length <= 24, "invalid WAX address");
        require(IGenericERC20(GE20).transferFrom(address(msg.sender), address(this), amount), 'transfer into contract');
        ...
        _totalStakes = IGenericERC20(GE20).balanceOf(address(this));
    ...
    function withdraw(uint256 amount) external nonReentrant returns (uint256, uint256){
        ...
        require(IGenericERC20(GE20).transfer(msg.sender, amount), 'transfer out of contract');
